...
\begin{document}
	...
	\subsection{Procedimiento de análisis}\label{sec:procedimiento-analisis}
	...
	El análisis humano es principalmente descriptivo: se contabilizan estados de tarea y se identifican requisitos que requieren revisión o refinamiento. No se aplicó SUS y, por tanto, no se reporta ninguna puntuación SUS. Como comprobación cuantitativa complementaria de P1, se construyó una tabla de contingencia 3$\times$2 (grupo de área $\times$ presencia de hallazgo) sobre las 72 relaciones codificadas por área. Se verificó el supuesto de frecuencia esperada mínima $\geq 5$ requerido por la prueba $\chi^2$ de Pearson asintótica; al no cumplirse (frecuencia esperada mínima = 0,5), se aplicó como alternativa una prueba de permutación Monte Carlo con 100\,000 réplicas sobre el mismo estadístico $\chi^2$, semilla aleatoria fija (42) para reproducibilidad, y se reportó el tamaño del efecto mediante la V de Cramér, con su precisión estimada mediante bootstrap no paramétrico (10\,000 réplicas, semilla 42, percentiles 2,5/97,5); por estar acotada en $[0,1]$, este intervalo no se usa como prueba de significancia, papel que cumple la prueba de permutación.
	Como referencia sobre la adecuación muestral del componente humano de RQ1, se calculó a priori la potencia estadística requerida para un contraste bilateral de dos muestras independientes con tamaño de efecto convencional pequeño-mediano (Cohen's $d=0{,}5$), $\alpha=0{,}05$ y potencia objetivo de $0{,}80$: el cálculo arroja $n=64$ por grupo ($n=128$ en total) \cite{faul2007}. Las diez sesiones de validación disponibles al corte no alcanzan ese tamaño; este cálculo es, por tanto, una referencia de diseño y no una afirmación de que la muestra actual tenga potencia suficiente para el contraste de P1, limitación que se retoma en la Sección \ref{sec:amenazas}.
	Para P2, un RF Must se contabiliza como cubierto únicamente cuando la prueba técnica automatizada correspondiente pasa sobre la versión final del MVP con datos sintéticos. El script y la salida en formato JSON se conservan en \texttt{06\_Experimento/scripts\_analisis/} y \texttt{06\_Experimento/resultados/}. El protocolo fue prerregistrado en OSF (DOI: 10.17605/OSF.IO/DTYNC); las desviaciones de ejecución respecto del plan prerregistrado, incluida la presente, se documentan en \texttt{osf\_deviations.pdf}.
	...
	La comprobación cuantitativa de la proposición P1 sobre las 72 relaciones observación-requisito codificadas por área arrojó $\chi^2(2)=1{,}99$. Dado que la frecuencia esperada mínima de la tabla de contingencia fue 0,5 (por debajo del umbral de 5 exigido para la prueba asintótica), se reporta el resultado de la prueba de permutación Monte Carlo aplicada como alternativa: $p=0{,}413$ (100\,000 réplicas, semilla 42), con un tamaño de efecto pequeño (V de Cramér $=0{,}166$; precisión de la estimación mediante bootstrap no paramétrico, IC 95\% $[0{,}084,\ 0{,}262]$, 10\,000 réplicas). Dado que la V de Cramér está acotada en $[0,1]$, este intervalo nunca incluye el cero por construcción y no debe interpretarse como una prueba de significancia: la prueba de significancia sobre la asociación es la prueba de permutación reportada arriba ($p=0{,}413$); el intervalo aquí solo describe la precisión de la estimación puntual del tamaño de efecto. Adicionalmente, 2 de las 6 celdas de la tabla de contingencia $3\times2$ (el subgrupo administrativo y el de paciente simulado en la columna ``sin hallazgo'') tienen frecuencia observada igual a cero, lo que limita la potencia de esta comprobación sobre una muestra de este tamaño. No se encontró evidencia de que la proporción de tareas con hallazgos de mejora difiera entre las áreas administrativas, clínicas y la simulación de paciente; es decir, la necesidad de refinamiento identificada durante el walkthrough es transversal a los roles y no se concentra en un área particular.
	...
	\begin{table}[H]
		\centering
		\caption{Cobertura técnica de los 22 RF Must}
		\begin{tabular}{cll}
			\hline
			RF & Requisito & Estado \\\hline
			RF-01 & Registrar historia clínica digital & PASA \\
			RF-02 & Agendar cita médica & PASA \\
			RF-03 & Buscar pacientes por criterios & PASA \\
			RF-06 & Cancelar / modificar cita médica & PASA \\
			RF-09 & Autenticación de usuarios & \textbf{NO PASA} \\
			RF-10 & Gestionar permisos y roles & PASA \\
			RF-11 & Enviar notificaciones automáticas & PASA \\
			RF-12 & Registrar auditoría de acciones & PASA \\
			RF-13 & Consultar disponibilidad de horarios & PASA \\
			RF-17 & Gestionar perfil del personal médico & PASA \\
			RF-18 & Asignar/consultar turnos del personal & \textbf{NO PASA} \\
			RF-19 & Consultar disponibilidad del personal & PASA \\
			RF-21 & Emitir comprobante interno de pago & PASA \\
			RF-22 & Registrar pagos totales o parciales & PASA \\
			RF-24 & Configurar horarios por área médica & PASA \\
			RF-26 & Signos vitales y atención de enfermería & PASA \\
			RF-27 & Búsqueda de paciente por cédula & PASA \\
			RF-28 & Registro de datos personales del paciente & PASA \\
			RF-30 & Restricción de acceso por rol & PASA \\
			RF-33 & Registrar cobro por área de atención & PASA \\
			RF-34 & Asignación de turno de atención & PASA \\
			RF-38 & Consulta rápida de paciente registrado & PASA \\ \hline
		\end{tabular}
	\end{table}
	...
	El cálculo de potencia estadística a priori para este contraste se reporta en la Sección de Metodología (\S\ref{sec:procedimiento-analisis}) --- ver también las limitaciones derivadas en la Sección \ref{sec:amenazas}.
